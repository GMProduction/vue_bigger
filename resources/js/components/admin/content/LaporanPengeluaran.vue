    ...
    <v-app>
      <div class="d-flex">
        ...
        <b-dropdown dropleft class="mr-2">
          ...
          <Button
            @click.native="loadDataPengeluaran()"
            text="Cari"
            style="height: 40px;"
            :bg-color="'bg-success'"
...
export default {
  ...
  methods: {
    ...
    loadDataPengeluaran() {
      ...
      this.axios
        .post(
          this.$store.state.apiUrl + 'pengeluaran/pencarianLaporan',
          this.inputPencarian,
          {
            headers: {
              Authorization: 'Bearer ' + this.$store.state.biggertoken,
        ...
        .then((response) => {
          console.log(response.data.pengeluaran);
          this.tablePembayaran = response.data.pengeluaran
