<template>
    <div class="home">
        ...
        <div class="d-flex flex-row mb-3">
            ...
            <Button @click.native="showTambahModalPesanan" :icondepan="'mdi-plus'" text="Tambah Data"
                :bg-color="'bg-success'" class="ml-2"></Button>
        </div>
        ...
        <div class="card-table">
            ...
            <v-app v-else>
                <v-data-table :headers="headers" :items="items" :search="search" :custom-filter="filterOnlyCapsText">
                    <template v-slot:top>
                        <v-text-field v-model="search" label="Cari disini (huruf kecil)" class="mx-4"></v-text-field>
                    </template>
                    <template v-slot:item.total_biaya="{ item }">
                        {{ formatPrice(item.total_biaya) }}
                    </template>
                    <template v-slot:item.actions="{ item }">
                        <v-btn v-if="item.status_proses == 'menunggu'" x-small class="mx-1" fab dark color="success"
                            @click="
                                showStatusModal(
                                    item.id,
                                    'selesai',
                        ...
                        <v-btn v-else x-small class="mx-1" fab dark color="red" @click="
                            showStatusModal(
                                item.id,
                                'menunggu',
                        ...
                        <v-btn x-small class="mx-1" fab dark color="warning" @click="
                        ...
                        <v-btn x-small class="mx-1" fab dark color="red" @click="showHapusModal(item.id, 'pesamam')">
                            <v-icon dark>
                                mdi-delete-outline
                            </v-icon>
                        </v-btn>
        ...
        <b-modal no-close-on-backdrop ref="add-pesanan-modal" size="xl" hide-footer :title="judulModal">
            <div v-if="loading" class="text-center m-5">
                <b-spinner variant="primary" label="Text Centered" class="mb-3"></b-spinner>
                <p>Tunggu Sebentar Ya...</p>
            </div>
            ...
            <v-app v-else>
                <v-card outlined class="rounded-xl mb-5">
                    <Button @click.native="showModalKeranjang" :icondepan="'mdi-plus'" text="Tambah Data Keranjang"
                        :bg-color="'bg-primary'" class="ml-2 mt-3 mb-2"></Button>

                    <v-data-table :headers="headersKeranjang" :items="tabelKeranjang" :search="search"
                        :custom-filter="filterOnlyCapsText">
                        <template v-slot:top>
                            <v-text-field v-model="search" label="Cari disini (huruf kecil)" class="mx-4">
                            </v-text-field>
                        </template>
                        ...
                        <template v-slot:item.actions="{ item }">
                            <v-btn v-if="item.status_pengerjaan == 'menunggu'" x-small class="mx-1" fab dark
                                color="success" @click="
                                    showStatusModal(
                                        item.id,
                                        'selesai',
                            ...
                            <v-btn x-small class="mx-1" fab dark color="red"
                  ...
                  <v-card class="rounded-xl p-3 mb-3">
                    ...
                    <v-radio-group v-model="pesAkun" mandatory>
                      ...
                      <v-radio label="R" value="r" ></v-radio>
                                ...
                                <v-col cols="12">
                                    <div class="d-flex">
                                        <v-autocomplete :items="itemsPelanggan" :filter="customFilterPelanggan" outlined
                                            dense v-model="pPelanggan" :error-messages="pPelangganError"
                                            class="rounded-lg mr-2" item-text="nama_pelanggan" item-value="id"
                                            label="Pelanggan" @change="pilihPelanggan">
                                            {{ namaPelanggan }}
                                        </v-autocomplete>

                                        <Button @click.native="showTambahDataPelanggan" type="button"
                                            :icondepan="'mdi-plus'" :bg-color="'bg-primary'" style="height: 40px;"
                                            class="rounded-lg"></Button>
                                    </div>

                                    <v-menu ref="menu" v-model="menu" :close-on-content-click="false"
                                        :return-value.sync="pesDeadline" transition="scale-transition" offset-y
                                        min-width="auto">
                                        <template v-slot:activator="{ on, attrs }">
                                            <v-text-field v-model="pesDeadline" label="Deadline"
                                                append-icon="mdi-calendar" readonly dense class="rounded-lg" outlined
                            ...
                            <v-textarea dense v-model="pesCatatan" class="rounded-lg" :counter="255" outlined
                                label="Catatan">
                                {{ pesCatatan }}
                            </v-textarea>
                        </v-col>
                        ...
                        <v-col cols="12" md="4">
                            <v-card class="rounded-xl p-3">
                                <p>Total Biaya</p>
                                <vuetify-money dense v-model="pesSubTotal" class="rounded-lg" outlined label="Sub Total"
                                    disabled v-bind:valueWhenIsEmpty="valueWhenIsEmpty" v-bind:options="options">
                                    {{ pesSubTotal }}
                                </vuetify-money>

                                <v-text-field dense v-model="pesDiskon" class="rounded-lg" :counter="11" outlined
                                    type="number" label="Diskon" @blur="hitungTotalPesanan">
                                    {{ pesDiskon }}
                                </v-text-field>

                                <vuetify-money dense v-model="pesTotal" class="rounded-lg" outlined disabled
                                    label="Total" v-bind:valueWhenIsEmpty="valueWhenIsEmpty" v-bind:options="options">
                                    {{ pesTotal }}
                                </vuetify-money>
                            </v-card>
                        </v-col>
                    ...
                    <div class="mt-3 d-flex flex-row">
                        ...
                        <div v-else class="w-100">
                            <v-alert v-show="inputError" :value="true" color="error" icon="mdi mdi-alert-outline"
                            ...
                            <div class="mt-3 d-flex flex-row">
                                <div class="flex-grow-1"></div>
                                <Button type="submit" :icondepan="'mdi-content-save-outline'" text="Simpan"
                                    :bg-color="'bg-success'" :color="'text-white'"></Button>
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <b-modal no-close-on-backdrop ref="add-keranjang-modal" size="md" hide-footer :title="judulModal">
            <v-app>
                <form @submit.prevent="InputDataKeranjang">
                    <div class="d-flex">
                        <v-autocomplete :items="itemsJenisBarang" :filter="customFilterJenisBarang" outlined dense
                            v-model="kJenisBarang" :error-messages="idJenisBarangError" class="rounded-lg mr-2"
                            item-text="nama" item-value="id" label="Jenis Barang" @change="pilihJenisBarang">
                            {{ kNamaJenisBarang }}
                        </v-autocomplete>

                        <Button @click.native="showTambahDataJenisBarang" type="button" :icondepan="'mdi-plus'"
                            :bg-color="'bg-primary'" style="height: 40px; color: #fff;" class="rounded-lg"></Button>
                    </div>

                    <v-text-field v-model="kUkuran" class="rounded-lg" :counter="50" outlined :suffix="satuan"
                        label="Total Ukuran" type="number" v-bind:valueWhenIsEmpty="valueWhenIsEmpty"
                        v-bind:options="options">
                        {{ kUkuran }}
                    </v-text-field>

                    <vuetify-money v-model="kQty" class="rounded-lg" :counter="50" outlined type="number" label="Qty"
                        @blur="hitungTotalKeranjang" v-bind:valueWhenIsEmpty="valueWhenIsEmpty"
                        v-bind:options="options">
                        {{ kQty }}
                    </vuetify-money>

                    <v-text-field v-model="kBiayaTambahan" class="rounded-lg" :counter="50" outlined type="number"
                        label="Biaya Tambahan" @blur="hitungTotalKeranjang">
                        {{ kBiayaTambahan }}
                    </v-text-field>

                    <vuetify-money v-model="kTotal" class="rounded-lg" :counter="50" outlined disabled label="Total"
                        v-bind:valueWhenIsEmpty="valueWhenIsEmpty" v-bind:options="options">
                        {{ kTotal }}
                    </vuetify-money>

                    <v-textarea v-model="kCatatan" class="rounded-lg" :counter="50" outlined label="Catatan">
                        {{ kCatatan }}
                    </v-textarea>
        ...
        <b-modal ref="add-jenisBarang-modal" hide-footer :title="judulModal">
            <v-app>
                <form @submit.prevent="InputDataJenisBarang">
                    <v-text-field v-model="jNama" class="rounded-lg" :error-messages="jNameErrors" :counter="50"
                        outlined label="Nama Jenis Barang" @input="$v.jNama.$touch()" @blur="$v.jNama.$touch()">
                    </v-text-field>

                    <v-text-field v-model="jSatuan" class="rounded-lg" :error-messages="jSatuanErrors" :counter="25"
                        outlined label="Satuan" @input="$v.jSatuan.$touch()" @blur="$v.jSatuan.$touch()"></v-text-field>

                    <v-text-field v-model="jHarga" class="rounded-lg" :error-messages="jHargaErrors" :counter="11"
                        outlined type="number" label="Harga" @input="$v.jHarga.$touch()" @blur="$v.jHarga.$touch()">
                    </v-text-field>
        ...
        <b-modal ref="add-pelanggan-modal" hide-footer :title="judulModal">
            <v-app>
                <form @submit.prevent="inputDataPelanggan" enctype="multipart/form-data">
                    <v-text-field v-model="pNama" class="rounded-lg" :error-messages="pNamaErrors" :counter="50"
                        outlined label="Nama Pelanggan" @input="$v.pNama.$touch()" @blur="$v.pNama.$touch()">
                    </v-text-field>

                    <v-text-field v-model="pNotelp" class="rounded-lg" :error-messages="pNotelpErrors" :counter="25"
                        outlined label="no_telp" @input="$v.pNotelp.$touch()" @blur="$v.pNotelp.$touch()">
                    </v-text-field>

                    <v-text-field v-model="pAlamat" class="rounded-lg" :error-messages="pAlamatErrors" :counter="225"
                        outlined label="alamat" @input="$v.pAlamat.$touch()" @blur="$v.pAlamat.$touch()"></v-text-field>
            ...
            <v-app>
                ...
                <div class="mt-3 d-flex flex-row">
                    ...
                    <div v-else class="w-100">
                        ...
                        <div class="mt-3 d-flex flex-row">
                            <div class="flex-grow-1"></div>
                            <Button class="mr-3" text="Ya" :bg-color="'bg-success'" :color="'text-white'"
                                @click.native="hapusData()"></Button>

                            <Button text="Tidak" :bg-color="'bg-danger'" :color="'text-white'"
                                @click.native="hideHapusModal()"></Button>
                        </div>
                    </div>
                </div>
            </v-app>
                ...
                <div class="mt-3 d-flex flex-row">
                    ...
                    <div v-else class="w-100">
                        ...
                        <div class="mt-3 d-flex flex-row">
                            <div class="flex-grow-1"></div>
                            <Button class="mr-3" text="Ya" :bg-color="'bg-success'" :color="'text-white'"
                                @click.native="UpdateStatusKeranjang()"></Button>

                            <Button text="Tidak" :bg-color="'bg-danger'" :color="'text-white'"
                                @click.native="hideStatusModal()"></Button>
                        </div>
                    </div>
                </div>
            </v-app>
...
export default {
    ...
    computed: {
        ...
        idJenisBarangError() {
            ...
            !this.$v.kJenisBarang.required &&
                errors.push('Harus memilih jenis barang.')
            return errors
        },
    ...
    methods: {
        ...
        hitungTotalKeranjang() {
            ...
            this.kTotal =
                parseInt(this.kUkuran * this.kQty * harga) +
                parseInt(this.kBiayaTambahan)
        },
        ...
        InputDataJenisBarang() {
            if (
                this.$v.jNama.$invalid ||
                this.$v.jHarga.$invalid ||
                this.$v.jSatuan.$invalid
            ) {
                this.submitStatus = 'ERROR'
                this.inputError = 'Periksa kembali isi data kamu'
            } else {
