    ...
    <div class="home">
        ...
        <div class="d-flex flex-row mb-3">
            <div style="width: 300px;">
                <v-menu ref="menutanggal" v-model="menutanggal" :close-on-content-click="false"
                    :return-value.sync="lapTanggal" transition="scale-transition" offset-y min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field v-model="lapTanggal" label="Tanggal" append-icon="mdi-calendar" readonly dense
                            class="rounded-lg" outlined v-bind="attrs" v-on="on"></v-text-field>
                    </template>
            ...
            <Button @click.native="showTambahData" :icondepan="'mdi-plus'" text="Tambah Data" style="height: 40px;"
        ...
        <div class="card-table">
            ...
            <v-app v-else>
                <v-data-table :headers="headers" :items="items" :search="search" :custom-filter="filterOnlyCapsText">
                    ...
                    <template v-slot:item.urlGambar="{ item }">
                        <a :href="'/img/pengeluaran/' + item.urlGambar" target="_blank">
                            <img :src="'/img/pengeluaran/' + item.urlGambar" alt="" width="50" />
                        </a>
                    </template>
                    ...
                    <template v-slot:item.actions="{ item }">
                        <v-btn x-small class="mx-1" fab dark color="warning"
                            @click="showEditModal(item.id, item.tanggal, item.nominal, item.akun, item.keterangan)">
                            <v-icon dark>
                                mdi-pencil-outline
                            </v-icon>
                        </v-btn>
                        <v-btn x-small class="mx-1" fab dark color="red" @click="showHapusModal(item.id, item.nama)">
                            <v-icon dark>
                                mdi-delete-outline
                            </v-icon>
                        </v-btn>
                ...
                <div class="mt-5">
                    <vuetify-money dense v-model="totalPengeluaran" class="rounded-lg" outlined label="Total" disabled
            ...
            <v-app>
                <form @submit.prevent="InputData">
                    <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="tanggal"
                        ...
                        <template v-slot:activator="{ on, attrs }">
                            <v-text-field v-model="tanggal" label="Tanggal" prepend-icon="mdi-calendar" readonly dense
                                :error-messages="tanggalErrors" class="rounded-lg" outlined v-bind="attrs" v-on="on">
                            </v-text-field>
                        </template>
                    ...
                    <vuetify-money v-model="nominal" class="rounded-lg" :error-messages="nominalErrors" :counter="11"
                        outlined type="number" label="Nominal" @input="$v.nominal.$touch()" @blur="$v.nominal.$touch()"
                        v-bind:valueWhenIsEmpty="valueWhenIsEmpty" v-bind:options="options">
                        {{ nominal }}
                    </vuetify-money>

                    <v-file-input :rules="rules" accept="image/png, image/jpeg, image/bmp" placeholder="Pilih Gambar"
                        prepend-icon="mdi-camera" label="Bukti Nota Pengeluaran" outlined v-model="urlGambar">
                        {{ urlGambar }}
                    </v-file-input>


                    <v-textarea dense v-model="keterangan" class="rounded-lg" :counter="199" outlined
                        label="Keterangan">
                        {{ keterangan }}
                    </v-textarea>
